...
function views.homepage(self)
    ...
    if (table.getn(is_auth) ~= 0) then
        if (is_auth[1][3] == 1) then
            ...
            resp = self:render({isAuthorized=1, name = designer[1][5], status=1, photo = designer[1][9]})
        else
            local showroom = box.space.showroom.index.email:select({is_auth[1][2]})
            log.info(showroom[1][8])
            resp = self:render({isAuthorized=1, name = showroom[1][2], status=2, photo = showroom[1][8][1]})
        end
    else
        resp = self:render({isAuthorized=0})
    end
    resp.headers['Access-Control-Allow-Origin'] = 'file:///*'
    return resp
end
